...
@section('content')
    ...
    <body>
        <div class="main ">
            <div class="d-flex align-items-center justify-content-between gap-3 tr-second-nav px-3 py-2 heading mb-4">
                <div style="font-size: 20px;">Calibration Usage</div>
                <div style="font-size: 20px; gap:10px" class='d-flex'>
                    ...
                    <button type="button" class="btn btn-warning" id="submitCalibration">Submit</button>
                </div>
            </div>

            <div class="table-responsive" style="padding-right: 1rem;">
                <table class="stock-planner-table" id="calibrationUsageTable">
                    <thead>
                        <tr>
                            <th rowspan="2">
            ...
            <!-- Modal Template (will be cloned for each instance) -->
    ...
    <script>
        function callmodel() {
            ...
            //         const modalTemplate = $('#modalTemplate').html();
            //         const modalClone = $(modalTemplate).clone().appendTo('body');
            //         modalClone.show();

            //         // Close modal when clicking the close button
            //         modalClone.find('.close').click(function () {
            //             modalClone.remove();
            //         });
            ...
            //         modalClone.find('.confirm-selection').click(function () {
            //             alert("Selection confirmed!"); // Modify this to handle your logic
            //             modalClone.remove();
            //         });
            //     });
            // });
        ...
        $(document).ready(function () {
            ...
            function initializeModals() {
                ...
                $(document).on('click', '.open-modal-btn', function () {
                    ...
                    const parameter = parametersData.find(p => p.calibration_name === parameterName);
            ...
            $('#labDropdown').change(function () {
                ...
                if (!currentLabId) {
                    $('#calibrationUsageTable tbody').empty();
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            parametersData = response.data.calibration_setup || [];
                            buildCalendarTable();
                        } else {
            ...
            function buildCalendarTable() {
                ...
                for (const [department, params] of Object.entries(departments)) {
                    ...
                    params.forEach(param => {
                        // Scheduled row
                        tableBody += `<tr><td rowspan="2">${param.calibration_name}</td>`;
                ...
                $('#calibrationUsageTable tbody').html(tableBody);
                initializeModals();
            }

            // Submit calibration usage data
            $('#submitCalibration').click(function () {
                if (!currentLabId) {
                ...
                $('#calibrationUsageTable tbody tr').each(function () {
                ...
                if (!hasData) {
                    alert('No calibration usage data to submit.');
                    return;
                }

                $.ajax({
                    url: '/save-calibration-usage',
                    type: 'POST',
                    data: {
                        lab_id: currentLabId,
