    ...
    <body>
        <div class="main ">
            <div class="d-flex align-items-center justify-content-between gap-3 tr-second-nav px-3 py-2 heading mb-4">
                <div style="font-size: 20px;">Control - Parameter Tagging</div>
                        ...
                        <table id="ControlParameterUsageTable">
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        <span
                                            style="border-radius:3px;border: 2px solid lightgray; padding:10px ;">12</span>
                                        X <span
                                            style="border-radius:3px;border: 2px solid lightgray; padding:10px">12</span>
                                        <span
                                            ...
                                            <div class="ilc-modal">
                                                <div class="ilc-modal-content">
                                                    <input type="text" id="searchInput" class="search-input"
                                                        ...
                                                        placeholder="Search for Test Codes or Names"
                                                        onkeyup="searchTable()">
                                                                            ...
                                                                            <tr>
                                                                                <td>
                                                                                    <select class="form-control department">
                                                                                        ...
                                                                                        ${data && data.departments2 ? data.departments2.map(dept => `<option value="${dept}">${dept}</option>`).join('') : ''}
                                                                                    </select>
                                                                                </td>
                                                                                <td>
                                                                                    <select class="form-control manufacturer">
                                                                                        <option value="">-- Select Manufacturer --</option>
                                                                                        ${data && data.manufacturers2 ? data.manufacturers2.map(manu => `<option value="${manu}">${manu}</option>`).join('') : ''}
                                                                                    </select>
                                                                                </td>
                                                                                <td>
                                                                                    <select class="form-control control-name">
                                                                                        <option value="">-- Select control Name --</option>
                                                                                        ${data && data.manufacturers2 ? data.manufacturers2.map(cal => `<option value="${cal}">${cal}</option>`).join('') : ''}
                                                                                    </select>
                                                                                </td>
                                                                                <td>
                                                                                    <select class="form-control machine">
                                                                                        <option value="">-- Select Machine --</option>
                                                                                        ${data && data.machines2 ? data.machines2.map(machine => `<option value="${machine}">${machine}</option>`).join('') : ''}
                                                                                    </select>
                                                                                </td>
                                                                                <td>
                                                                                    <div class="instance">
        ...
        $(document).ready(function () {
            ...
            $('button.btn-warning').click(function () {
                ...
                $('#tableBody tr').each(function () {
                    ...
                    const controlManufacturer = row.find('.manufacturer').val();
                    const controlName = row.find('.control-name').val();
                    const suitableMachine = row.find('.machine').val();
                    const parameters = row.find('.selection').text(); // Get selected parameters


                    if (department && controlManufacturer && controlName && suitableMachine && parameters) {
                        rows.push({
                            department,
                            control_manufacturer: controlManufacturer,
                            control_name: controlName,
                            suitable_machine: suitableMachine,
                            parameters: JSON.stringify(parameters.split(', ')), // Convert to JSON
                        });
                    }
                });
                ...
                $.ajax({
                    url: '/control-parameter-tagging/store',
                    type: 'POST',
                    headers: {
