...
class EditBusinessInfo extends Component
{
    ...
    public function mount(Request $request)
    {
        ...
        $bus_intl = BusinessInteligence::with('expectedBusiness')->where('lead_id', $this->reportId)->first();

        $this->expected_business = $bus_intl ? $bus_intl->expectedBusiness->toArray() : [];
    ...
    public function deleteReport($reportId)
    {
        ...
        // Refresh reports
        $this->list_contact_details = ContactDetail::where('lead_id', $this->reportId)->orderBy('id', 'desc')->paginate(5);
    ...
    public function addNewField()
    {
        $this->expected_business[] = [
            'id' => null, // Initialize as null for a new entry
            'test_name' => '',
            'expected_qty_day' => '',
            'expected_l2l_price' => '',
            'amount' => '',
    ...
    public function removeField($index)
    {
        // Check if the index exists before trying to unset it
        if (isset($this->expected_business[$index])) {
            // Get the corresponding expected business entry
            $business = ExpectedBusiness::find($this->expected_business[$index]['id']);
    ...
    public function submitForm()
    {
        ...
        \DB::transaction(function () {
            ...
            $existingBusinesses = $bus_intl->expectedBusiness->keyBy('test_name');
            ...
            foreach ($this->expected_business as $business) {
                if (!empty($business['test_name']) && !empty($business['expected_qty_day']) && !empty($business['total']) && !empty($business['expected_l2l_price']) && !empty($business['amount'])) {
                    ...
                    if ($existingBusinesses->has($business['test_name'])) {
                        ...
                        // Insert new expected business if it doesn't exist
                        $bus_intl->expectedBusiness()->create([
                            ...
                            'amount' => $business['amount'],
                            'total' => $business['total'],
                            'lead_id' => $report->id,  // Include lead_id here if necessary
                        ]);
                    }
                }
            }
    ...
    {
        $contactDetails = ContactDetail::paginate(5); // This handles pagination directly
        return view('livewire.edit-business-info', ['contactDetails' => $contactDetails]);
    }
